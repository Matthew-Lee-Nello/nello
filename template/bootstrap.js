#!/usr/bin/env node
/**
 * nello bootstrap - renders the install bundle into a live installation.
 *
...
// This means bypassPermissions only applies when Claude is working in ~/nello/.
...
const LAUNCHAGENT_LABEL = process.env.NC_LAUNCHAGENT_LABEL || 'com.nello.server'
...
function buildContext(bundle) {
  ...
  return {
    ...
    mcps: bundle.mcps || {},
    mcpsList,
    // Semantic recall (gbrain) master switch. An OPENAI_API_KEY in .env is the
    ...
    // (v1.0 swapped embeddings off Voyage onto OpenAI text-embedding-3-small -
    // cheaper, faster, and one key the install already collects.)
    brainEnabled: !!((bundle.keys || {}).OPENAI_API_KEY),
  }
}
...
function writeEnv(bundle) {
  ...
  // Semantic recall (gbrain). An OPENAI_API_KEY is the master switch: when present,
  ...
  if (keys.OPENAI_API_KEY && !keys.OPENAI_API_KEY.startsWith('sk-')) {
    warn('OPENAI_API_KEY does not start with "sk-" - double-check it at platform.openai.com. A wrong key flips recall on but 401s silently; /install-doctor catches it.')
  }
  ...
  const lines = [
    '# Auto-generated by nello bootstrap',
...
// Servers nello shipped before the Composio migration. The current renderer
...
// Kept tight to the names nello itself shipped, so a client's own MCPs are
// never touched.
...
function createMacAppShortcut() {
  const appDir = join(homedir(), 'Applications', 'nello.app')
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>com.nello.app</string>
  <key>CFBundleName</key><string>nello</string>
  <key>CFBundleDisplayName</key><string>nello</string>
...
function createWindowsShortcuts() {
  ...
  const startMenu = join(process.env.APPDATA || join(homedir(), 'AppData', 'Roaming'), 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'nello.lnk')
  const desktop = join(homedir(), 'Desktop', 'nello.lnk')
...
// Installed ONLY when an OPENAI_API_KEY is present (ctx.brainEnabled). gbrain is a
...
// no-ops, the graphify hook skips silently). OpenAI (text-embedding-3-small) is the
// only key gbrain needs on the import/query path.
...
function configureGbrain(openaiKey) {
  // `gbrain init --pglite` lays down a valid default config + local PGLite store.
  // Then point the embedding model at OpenAI (text-embedding-3-small, 1536-dim) so
  // embeds need only OPENAI_API_KEY. We patch just those fields and preserve gbrain's
  // other defaults. (v1.0: was voyage:voyage-3.5-lite/1024 - the dimension change is
  // why migration 0002 re-imports the vault when an old install switches over.)
  ...
  try {
    ...
    ok('gbrain configured (openai:text-embedding-3-small, local PGLite)')
  } catch (err) {
...
// notes; embedding all of them is an OpenAI cost event the owner should trigger
...
function seedRecall(ctx) {
  ...
  if (count > 800) {
    warn(`Vault has ${count}+ notes - skipping auto-embed to avoid a large OpenAI bill. Run /build-recall to embed it when ready.`)
    return
  }
  try {
    execSync(`"${GBRAIN_BIN}" import "${vault}"`, {
      stdio: 'pipe',
      env: { ...process.env, HOME: homedir(), OPENAI_API_KEY: ctx.env.OPENAI_API_KEY || '' },
    })
...
function setupRecall(ctx) {
  ...
  installGraphify()
  configureGbrain(ctx.env.OPENAI_API_KEY)
  seedRecall(ctx)
}
...
function detectStaleInstalls() {
  ...
  // into a fresh folder without cleaning up ~/nello and the schtasks/launchd
  // entry from a prior attempt, which fights the new daemon on next logon.
  const here = resolve(INSTALL_PATH)
  const dirCandidates = [join(homedir(), 'nello')].filter(p => existsSync(p) && resolve(p) !== here)
...
async function main() {
  ...
  if (!hasClaudeLogin) {
    ...
    if (process.platform === 'darwin') {
      ...
      console.log(`  ${DIM}Log out and back in (the Startup item relaunches the daemon), or kill the node daemon and re-run nello-daemon.cmd${RESET}`)
    } else {
