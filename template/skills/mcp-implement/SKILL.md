...
3. **No secrets in `CLAUDE.md` or the config.** API keys go in `.env`; reference them by env var in the server's `env` block.
...
Add any new secret to `<install-folder>/.env` and reference it by name. Example (pinned stdio):
```json
"slack": {
...
```
> The installer (`bootstrap.js`) preserves servers you add here on any future re-run — it merges its baseline over your additions rather than overwriting them.

### Step 5 — Collect credentials
Walk the owner through getting whatever the server needs (API key, OAuth token), the same paste-and-validate way the install does it. Write the value into `.env`. Never echo a full secret back.
...
- **Windows:** stop + start the `com.nello-claw.server` scheduled task (or the Startup-folder launcher).
